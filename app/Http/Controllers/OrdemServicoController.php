...
class OrdemServicoController extends Controller
{
    public function index()
    {
        ...
        try {
            $ordensServicos = OrdemServico::with(['produtos', 'servicos', 'funcionarios', 'cliente'])->get();
    ...
    public function update(Request $request)
    {
        ...
        $ordensServicos = OrdemServico::findOrFail($request->id);

        $ordensServicos->nome = $request->input('nome');
        $ordensServicos->codigoInterno = $request->input('codigoInterno');
        $ordensServicos->valor = $request->input('valor');
        $ordensServicos->comissao = $request->input('comissao');
        $ordensServicos->descricao = $request->input('descricao');

        try {
            $ordensServicos->save();
            $response = APIHelper::APIResponse(true, 200, 'Sucesso ao cadastrar o serviço', $ordensServicos);
            return response()->json($response, 200);
        } catch (Exception  $ex) {
            $response = APIHelper::APIResponse(false, 500, null, null, $ex);
            return response()->json($response, 500);
